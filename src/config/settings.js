...
(function () {
  ...
  const DEFAULTS = {
    ...
    promptsVersion: 11,            // v11：解析层加 normalizeJSONString 状态机预处理（治 DeepSeek 字符串内真实换行/中文标点/尾随逗号三类结构污染），parseRelations/parsePlots/parseItems/parseWorld/taggedSummary 全部补中文字段别名（从/到/关系/名字/描述/持有者 等），extractArray 候选键补中文（关系/剧情/物品）。prompt 仅给 summary/items 加换行转义提醒服务思考模型。低于此版本自动覆盖用户旧提示词。
    prompts: {
      // ═══════════════════════════════════════════
      // 输出契约（v8 统一约定）：
