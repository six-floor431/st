...
(function () {
  ...
  const DEFAULTS = {
    ...
    promptsVersion: 8,             // v8：重写全部提示词 + 解析层。删除七层正则枚举，统一 isJunkText 单点过滤；删除所有文本回退路径。低于此版本自动覆盖用户旧提示词。
    prompts: {
      // ═══════════════════════════════════════════
      // 输出契约（v8 统一约定）：
