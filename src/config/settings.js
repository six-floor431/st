...
(function () {
  ...
  const DEFAULTS = {
    ...
    promptsVersion: 6,             // v6：重构全部提示词——正向契约式输出，移除「禁止XX」反模式与禁词清单（v5 头号污染源），字段长度与解析层逐一校准。低于此版本自动覆盖用户旧提示词。
    prompts: {
      // ═══════════════════════════════════════════
      // 输出契约（v6 统一约定）：
