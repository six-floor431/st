(() => {
  ...
  (function() {
    ...
    const DEFAULTS = {
      ...
      promptsVersion: 12,
      // v12：提示词针对 DeepSeek「过度发挥」特性重写规范层——summary 锚定「记录员不是小说家」并列举氛围/心理反例、段数放宽；relations 加称呼归一与 label 正反例、排除「提到名字即关系」；plot 强化增量语义（旧条目一律不重出）与 title 正反例；items 用正反例圈定「具象物件」排除地点/场景/抽象概念。四契约统一换行转义提醒。解析层 v11 的 normalizeJSONString + 中文字段容错继续作为兜底。低于此版本自动覆盖用户旧提示词。
      prompts: {
        // ═══════════════════════════════════════════
        // 输出契约（v8 统一约定）：
